...
def who_owns_thumbs_up
  ...
  execute(<<-SQL)
    CREATE INDEX idx_name_color
    ON toys(name, color);
    CREATE INDEX idx_color_name
    ON cats(color, name);
    CREATE INDEX idx_id_color_name
    ON cattoys(cat_id, toy_id);
    ...
    EXPLAIN
      ...
      FROM
        cats
      JOIN
        cattoys ON cats.id = cattoys.cat_id AND cats.color = 'Pink'
      JOIN
        toys ON toys.id = cattoys.toy_id AND toys.name = 'Thumbsup'
  SQL
end
