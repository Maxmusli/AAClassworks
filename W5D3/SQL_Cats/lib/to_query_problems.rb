...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    SELECT
      all_toys.name, all_toys.price
    FROM 
      toys as shiny_mouse
      JOIN
        toys as all_toys on all_toys.price = shiny_mouse.price AND shiny_mouse.name = 'Shiny Mouse'
    WHERE
      all_toys.name != 'Shiny Mouse' 
  SQL
end
...
def just_like_orange
  ...
  execute(<<-SQL)
    SELECT
      all_cat.name, all_cat.breed
    FROM
      cats AS orange
    JOIN
      cats AS all_cat ON all_cat.breed = orange.breed AND orange.name = 'Orange'
    WHERE
      all_cat.name != 'Orange'
    ORDER BY
      all_cat.name
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
    SELECT
      DISTINCT all_cats.name, toys.name
    FROM 
      cats as some_cat
    JOIN
      cattoys AS jet_cattoys ON jet_cattoys.cat_id = some_cat.id AND some_cat.name = 'Jet'
    JOIN 
      toys AS toys ON toys.id = jet_cattoys.toy_id 
    JOIN
      cattoys as all_cattoys ON toys.id = all_cattoys.toy_id
    JOIN
      cats as all_cats ON all_cats.id = all_cattoys.cat_id
    WHERE
      all_cats.name != 'Jet'
    ORDER BY
      all_cats.name
