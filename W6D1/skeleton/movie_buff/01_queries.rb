...
def harrison_ford
  ...
  Movie.select(:id, :title).distinct.joins(:actors, :castings).where(actors: {name: 'Harrison Ford'}).where.not(castings: {ord: 1})
end
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:actors, :castings).group(:id).order('COUNT(castings.movie_id) desc').limit(3)
end
...
def directed_by_one_of(them)
  ...
  Movie
    ...
    .where(director_id: Actor.where(name: them).select(:id))
end
